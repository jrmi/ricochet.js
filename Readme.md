...
*x-ricochet-origin*, *referer*, *origin*. Those header are used to determine the website
where the backend code is stored. Let's call this `<ricochetOrigin>` in this document. By default
if you use a classic *browser*, *referer* or origin should be included automatically.

On the first on any API call for a specific *siteId*, the file
`<ricochetOrigin>/setup.js` will be downloaded.

This is the encrypted server side bundle that will be cached and
executed to configure the corresponding *siteId*.
...
This is the store instance scoped to the current site. You have access to the
following methods:
